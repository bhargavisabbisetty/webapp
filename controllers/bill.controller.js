        ...
        if(results.length == 0){
            ...
            if(bill.owner_id != user.id){
                ...
                for (var status in PaymentStatusEnum) {
                    ...
                    response.status(200).json({message: "Bill Found",bill});
                    break
                    }      
            }
            }
                        ...
                        if (!isNaN(amount_due)) {
                            if (amount_due >= 0.01) {
                                ...
                                if (validatorObj.isValidStatus(paymentStatus, PaymentStatusEnum)) {
                                    ...
                                    server.connection.query('UPDATE billdetails SET updated_ts = ?, vendor = ?, bill_date = ?, due_date = ?, amount_due = ?, categories = ?, paymentStatus = ? WHERE id = ? and owner_id = ?', params, function (error, results, fields) {
                                        response.status(200).json({
    ...
    server.connection.query('SELECT * from billdetails where id = ?', billId, function (error, results, fields) {
        if(results.length == 0){
            ...
            if(bill.owner_id != user.id){
                ...
                server.connection.query('DELETE from billdetails where id = ? and owner_id = ?', [billId, user.id], function (error, results, fields) {
